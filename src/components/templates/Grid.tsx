...
import { getNewStartEndTime } from "@/lib/utils";
...
function getNewStartEndTime(
  offset: number, // offset in pixels
  start: Date,
  end: Date,
  day: Date,
): { start: Date; end: Date } {
  const offsetInQuarters =
    Math.ceil(offset / pixelPerQuarter) * pixelPerQuarter;

  const startTime = dayjs(start).format("HH:mm");
  const endTime = dayjs(end).format("HH:mm");

  const startDateTime = dayjs(
    `${dayjs(day).format("YYYY-MM-DD")}T${startTime}`,
  );
  const endDateTime = dayjs(`${dayjs(day).format("YYYY-MM-DD")}T${endTime}`);

  const newStartDate = startDateTime.add(offsetInQuarters, "minute").toDate();
  const newEndDate = endDateTime.add(offsetInQuarters, "minute").toDate();
